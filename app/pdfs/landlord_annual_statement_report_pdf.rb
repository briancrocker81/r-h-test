...
## pdf page width 529
class LandlordAnnualStatementReportPdf < Prawn::Document
  PAGE_MARGIN = [40, 40, 40, 40]
  # legal - 612.00 x 1008.00
  def initialize(property_id, start_date, end_date)
    ...
    @property_expenses = @property.property_expenses.where(expense_date: @start_date..@end_date)
    if @property_expenses.nil?
      @property_expenses = 0.00
    end
    @property_income = TenancyPaymentItem.includes(:tenancy).where(tenancy_id: @property.tenancies.pluck(:id), due_date: @start_date..@end_date)
    if @property_income.nil?
      @property_income = 0.00
    end
  ...
  def statement_report
    text "<b>Period between #{@start_date.strftime('%d %B %Y')} and #{@end_date.strftime('%d %B %Y')}</b>", align: :center, size: 16, inline_format: true
    move_down 10
    ...
    text "<b>Property:</b> #{@property&.property_name}", inline_format: true
    move_down 10
    line_width 1
    stroke_horizontal_line(0, bounds.width)
    move_down 20
    text "<b>Income table</b>", inline_format: true, size: 16
    move_down 10
    t = make_table(get_property_income, cell_style: { border_width: 1, align: :center}, column_widths: 87) do
      style row(0), size: 12, background_color: "28a745", font_style: :bold
    end
    t.draw
    move_down 30
    text "<b>Expenses table - pull from property expenses from #{@start_date.strftime('%d %B %Y')} to #{@end_date.strftime('%d %B %Y')}</b>", inline_format: true, size: 16
    move_down 10
    t = make_table(get_property_expenses, cell_style: { border_width: 1, align: :center }, column_widths: 529/3)  do
      style row(0), size: 12, background_color: "28a745", font_style: :bold
    end
    t.draw
  ...
  def get_property_expenses
    table_data = [["Expenses", "Category", "Amount"]]
    if @property_expenses.count > 0
      @property_expenses.each do |expense|
        url = expense.file.url if expense.file.present?
        table_data << [expense.name, expense.category, "£#{number_with_delimiter(sprintf("%.2f",expense.amount.to_f), delimiter: ',')}"]
      end
    else
      table_data << [{content: "#{I18n.t :'reporting.no_expenses'}", colspan: 3, align: :left, font_style: :bold, text_color: "dc3545"}]
    end
    table_data
  end
